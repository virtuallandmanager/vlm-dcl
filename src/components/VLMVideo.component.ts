  ...
  export class DCLConfig extends VLMBase.MaterialConfig implements HasHybridTexture, Emissive, Audible, HasPlaylist, Playable {
    ...
    updateTexture: CallableFunction = (src: string) => {
      ...
      if (this.textureMode === SourceTypes.IMAGE) {
        ...
        const system = systems[this.sk];
        if (includes(this.playlist, system?.playlist[system.playlistIndex])) {
          return
        }
    ...
    updatePlaylist: CallableFunction = (config: VLMConfig) => {
      try {
        this.playlist = config.playlist;
        log('go fuck yourself bitch')
        // const system = systems[this.sk] || new VLMVideoSystem(this);
        // system.playlist = config.playlist;
        // log(system)
        // if (this.textureMode === SourceTypes.PLAYLIST && !includes(this.playlist, system.playlist[system.playlistIndex])) {
        //   // this.stop();
        //   // this.startPlaylist();
        // }
      } catch (error) {
