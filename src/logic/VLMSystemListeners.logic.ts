import { onEnterSceneObservable, onLeaveSceneObservable, onPlayerDisconnectedObservable, onPlayerClickedObservable, onPlayerExpressionObservable, onProfileChanged, onEnterScene, onLeaveScene } from '@dcl/sdk/observables'
  ...
  static init: CallableFunction = () => {
    try {
      ...
      onPlayerExpressionObservable.add(({ expressionId }) => {
        console.log(`VLM | SESSION ACTION: Emote triggered - ${expressionId}`);
      ...
      onPlayerClickedObservable.add((clickEvent) => {
        ...
        console.log("VLM | SESSION ACTION: Viewed A User Profile", clickEvent.userId);
      });
      ...
      //     console.log("VLM | SESSION ACTION: Witnessed Connection", userId);
      ...
      //     console.log("VLM | SESSION ACTION: Witnessed Disconnection", userId);
      ...
      onEnterSceneObservable.add(async ({ userId }) => {
        ...
        if (userId == this.sessionUser?.connectedWallet) {
          console.log("VLM | SESSION ACTION: Player Entered Scene Boundaries", userId);
          ...
          console.log("VLM | SESSION ACTION: Witnessed Player Enter Scene Boundaries", userId);
          let user = await getPlayerData({ userId });
          VLMEventManager.events.emit('VLMSessionAction', `Witnessed Scene Entry`, { userId, otherPlayers, witness: this.sessionUser.connectedWallet });
        }
      });
      ...
      onLeaveSceneObservable.add(async ({ userId }) => {
        ...
        if (userId == this.sessionUser?.connectedWallet) {
          ...
          let user = await getPlayerData({ userId });
          VLMEventManager.events.emit('VLMSessionAction', `Witnessed Scene Departure`, { userId, otherPlayers, witness: this.sessionUser.connectedWallet });
        }
      });
      ...
      VLMEventManager.events.on('VLMClaimEvent', (message: VLMClaimEvent) => {
        console.log("VLM - GIVEAWAY CLAIM - ", message)
      ...
      VLMEventManager.events.on('VLMSessionAction', ({ action, metadata }: VLMSessionAction) => {
        if (this.sessionData?.sessionToken) {
          ...
          console.log("VLM - LOGGED ANALYTICS ACTION - ", action, pathPoint, metadata);
        }
      });

      VLMEventManager.events.on('VLMSoundStateEvent', ({ elementData, userId }: VLMSoundStateEvent) => {
        const id = elementData.sk;
        console.log(id, VLMSound.configs[id]);
        console.log("VLM - SOUND STATE CHANGED", userId, this.sessionUser.sk);
      ...
      VLMEventManager.events.on('VLMPathClientEvent', (message: VLMPathClientEvent) => {
        console.log("VLM: Triggered client path event", message);
      ...
      VLMEventManager.events.on('VLMSceneMessage', (message: VLMSceneMessage) => {
        console.log("VLM - SCENE MESSAGE RECEIVED", message);
        switch (message.action) {
          case "init":
            VLMSceneManager.initScenePreset(message);
            console.log("VLM - SCENE INIT", message);
            break;
          case "create":
      ...
      VLMEventManager.events.on('VLMVideoStatusEvent', (message: VLMVideoStatusEvent) => {
        ...
        if (!videoId) {
          console.log("VLM - VIDEO STATE CHANGED - NO VIDEO ID", message)
          return;
        }
        ...
        if (videoConfig?.liveSrc == message.url) {
          console.log("VLM - VIDEO STATE CHANGED", message);
      ...
      VLMEventManager.events.on('VLMUserMessage', async (message: VLMUserMessage) => {
        if (message?.type == "inbound") {
          console.log("VLM - MESSAGE RECEIVED FROM USER", message)
      ...
      this.sceneRoom.onMessage("scene_preset_update", (message: VLMSceneMessage) => {
        console.log("Scene Preset Updated!", message);
        if (message.action) {
      ...
      this.sceneRoom.onMessage("scene_setting_update", (message: VLMSettingsEvent) => {
        console.log("Scene Setting Updated!", message);
      ...
      this.sceneRoom.onMessage("giveaway_claim_response", (message: VLMClaimEvent) => {
        console.log("Claim response received", message);
      ...
      this.sceneRoom.onMessage("request_player_position", (message: VLMPlayerPosition) => {
        console.log("Player Position Requested", message);
      ...
      console.log("VLM - ERROR - ERROR REGISTERING EVENT LISTENERS", e);
      throw e;
    }
  };
